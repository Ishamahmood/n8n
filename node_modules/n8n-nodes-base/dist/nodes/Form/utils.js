...
function prepareFormData({ formTitle, formDescription, formSubmittedText, redirectUrl, formFields, testRun, query, instanceId, useResponseData, appendAttribution = true, buttonLabel, }) {
...
async function prepareFormReturnItem(context, formFields, mode, useWorkflowTimezone = false) {
    ...
    if (Object.keys(workflowStaticData || {}).length &&
        context.getNode().type === n8n_workflow_1.FORM_TRIGGER_NODE_TYPE) {
        returnItem.json.formQueryParameters = workflowStaticData;
    }
    return returnItem;
}
function renderForm({ context, res, formTitle, formDescription, formFields, responseMode, mode, formSubmittedText, redirectUrl, appendAttribution, buttonLabel, }) {
...
async function formWebhook(context, authProperty = interfaces_1.FORM_TRIGGER_AUTHENTICATION_PROPERTY) {
    ...
    if (method === 'GET') {
        ...
        if (!redirectUrl && node.type !== n8n_workflow_1.FORM_TRIGGER_NODE_TYPE) {
            const connectedNodes = context.getChildNodes(context.getNode().name);
            const hasNextPage = connectedNodes.some((n) => n.type === n8n_workflow_1.FORM_NODE_TYPE || n.type === n8n_workflow_1.WAIT_NODE_TYPE);
            if (hasNextPage) {
