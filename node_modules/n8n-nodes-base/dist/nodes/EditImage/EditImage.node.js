            ...
            try {
                ...
                if (operations[0].operation !== 'create') {
                    ...
                    if (item.binary[dataPropertyName] === undefined) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Item does not contain any binary data with the name "${dataPropertyName}".`, { itemIndex });
                    }
                ...
                for (let i = 0; i < operations.length; i++) {
                    ...
                    if (operationData.operation === 'blur') {
                        ...
                        if (item.binary[operationData.dataPropertyNameComposite] === undefined) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Item does not contain any binary data with the name "${operationData.dataPropertyNameComposite}".`, { itemIndex });
                        }
                        const { fd, path, cleanup } = await (0, tmp_promise_1.file)();
                        cleanupFunctions.push(cleanup);
                        const binaryDataBuffer = await this.helpers.getBinaryDataBuffer(itemIndex, operationData.dataPropertyNameComposite);
