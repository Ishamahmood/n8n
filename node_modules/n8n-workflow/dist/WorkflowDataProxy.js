...
class WorkflowDataProxy {
    ...
    getDataProxy(opts) {
        ...
        const handleFromAi = (name, _description, _type = 'string', defaultValue) => {
            if (!name || name === '') {
                throw new expression_error_1.ExpressionError('Please provide a key', {
                    runIndex: that.runIndex,
                    itemIndex: that.itemIndex,
                });
            }
            ...
            const placeholdersDataInputData = that.runExecutionData?.resultData.runData[that.activeNodeName]?.[0].inputOverride?.["ai_tool"]?.[0]?.[0].json;
