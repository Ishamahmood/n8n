...
class RoutingNode {
    ...
    async runNode(inputData, runIndex, nodeType, executeData, nodeExecuteFunctions, credentialsDecrypted, abortSignal) {
        ...
        if (nodeType.description.credentials?.length) {
            if (nodeType.description.credentials.length === 1) {
                ...
                if (!credentialDescription) {
                    throw new node_operation_error_1.NodeOperationError(this.node, `Node type "${this.node.type}" does not have any credentials of type "${authenticationMethod}" defined`, { level: 'warning' });
                }
            }
        }
        let credentials;
        if (credentialsDecrypted) {
            ...
            try {
                credentials =
                    (await executeFunctions.getCredentials(credentialDescription.name)) || {};
            }
            catch (error) {
