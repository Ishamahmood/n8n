...
exports.workflowsController.get(`/`, __1.ResponseHelper.send(async (req) => {
    ...
    if (req.query.filter) {
        try {
            ...
            throw new __1.ResponseHelper.ResponseError('Failed to parse filter');
        }
    }
    const query = {
    ...
    if (req.user.globalRole.name === 'owner') {
        workflows = await __1.Db.collections.Workflow.find(Object.assign(query, {
            where: filter,
        }));
    }
    else {
        const shared = await __1.Db.collections.SharedWorkflow.find({
            relations: ['workflow'],
        ...
        workflows = await __1.Db.collections.Workflow.find(Object.assign(query, {
            where: {
                id: (0, typeorm_1.In)(shared.map(({ workflow }) => workflow.id)),
                ...filter,
            },
        }));
    }
...
exports.workflowsController.get('/:id', __1.ResponseHelper.send(async (req) => {
...
exports.workflowsController.patch(`/:id`, __1.ResponseHelper.send(async (req) => {
    ...
    await __1.WorkflowHelpers.replaceInvalidCredentials(updateData);
    __1.WorkflowHelpers.addNodeIds(updateData);
    await Server_1.externalHooks.run('workflow.update', [updateData]);
    ...
    if (updateData.settings) {
        ...
        if (parseInt(updateData.settings.executionTimeout, 10) ===
    ...
    await __1.Db.collections.Workflow.update(workflowId, updateData);
